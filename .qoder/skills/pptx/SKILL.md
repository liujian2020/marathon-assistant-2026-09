...
Save final output as `output.pptx` in the workspace, then copy to the outputs folder when delivering.
...
Copy `output.pptx` into the outputs folder, then share with a `file://` link and a one-paragraph note that lists which QA checks ran and any non-blocking caveats (e.g., "visual spot-check skipped for a 6-slide internal deck"). Do not write a long postamble.
...
Deliver the `.pptx`, a one-paragraph QA note, and a `file://` link. Never deliver a deck with P0 issues. State explicitly which inline QA checks ran and which were skipped.
